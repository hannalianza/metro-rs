      ...
      <section className="bg-warm-100 py-16" aria-labelledby="categories-heading">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {categories
              .filter((cat) => ["refrigeration", "cooking-equipment", "prep-tables", "food-prep", "sinks"].includes(cat.slug))
              .map((cat) => (
          ...
          <div className="mt-10 border-t border-warm-300 pt-10">
            <p className="mb-5 text-xs font-bold uppercase tracking-widest text-warm-500">Specialty Equipment</p>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {categories
                .filter((cat) => ["walk-in-coolers-freezers", "glass-door-merchandisers", "sushi-display-cases"].includes(cat.slug))
                .map((cat) => (
                  <Link key={cat.slug} href={`/products?category=${cat.slug}`} className="group flex items-center gap-4 rounded-xl border border-warm-200 bg-white p-4 transition-all hover:border-blue-300 hover:shadow-md">
                    <div className="h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-warm-100">
                      <img src={cat.image} alt={cat.name} className="h-full w-full object-contain p-2" loading="lazy" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm font-bold text-gray-900 group-hover:text-blue-800">{cat.name}</h3>
                      <p className="mt-0.5 text-xs text-gray-500 line-clamp-2">{cat.description}</p>
                    </div>
                    <svg className="ml-auto h-4 w-4 flex-shrink-0 text-gray-400 transition-transform group-hover:translate-x-1 group-hover:text-blue-800" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                    </svg>
                  </Link>
                ))}
